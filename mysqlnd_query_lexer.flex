...
%{
	...
	smart_string ** comment = &(yylval_param->comment);
...
-?"."[0-9]+E[-+]?[0-9]+			{ ZVAL_DOUBLE(token_value, atof(yytext)); DBG_INF("QC_TOKEN_FLOATNUM");	DBG_RETURN(QC_TOKEN_FLOATNUM); }
	...
	/* Normal strings */
'(\\.|''|[^'\n])*' |
\"(\\.|\"\"|[^"\n])*\"			{ ZVAL_STRINGL(token_value, yytext, yyleng); DBG_INF("QC_TOKEN_STRING");	DBG_RETURN(QC_TOKEN_STRING); }
'(\\.|[^'\n])*$					{ yyerror("Unterminated string %s", yytext); }
\"(\\.|[^"\n])*$				{ yyerror("Unterminated string %s", yytext); }

	/* Hex and Bit strings */
X'[0-9A-F]+' |
0X[0-9A-F]+						{ ZVAL_STRINGL(token_value, yytext, yyleng); DBG_INF("QC_TOKEN_STRING");	DBG_RETURN(QC_TOKEN_STRING); }
0B[01]+ |
B'[01]+'						{ ZVAL_STRINGL(token_value, yytext, yyleng); DBG_INF("QC_TOKEN_STRING");	DBG_RETURN(QC_TOKEN_STRING); }
...
":="							{ DBG_INF("QC_TOKEN_ASSIGN_TO_VAR");DBG_RETURN(QC_TOKEN_ASSIGN_TO_VAR); }
	...
	/* normal identifier */
([A-Za-z$_]|[\xC2-\xDF][\x80-\xBF]|\xE0[\xA0-\xBF][\x80-\xBF]|[\xE1-\xEF][\x80-\xBF][\x80-\xBF])([0-9]|[A-Za-z$_]|[\xC2-\xDF][\x80-\xBF]|\xE0[\xA0-\xBF][\x80-\xBF]|[\xE1-\xEF][\x80-\xBF][\x80-\xBF])* { ZVAL_STRINGL(token_value, yytext, yyleng); DBG_INF("QC_TOKEN_IDENTIFIER"); DBG_RETURN(QC_TOKEN_IDENTIFIER); }
...
`[^`/\\.\n]+`					{
	...
	/* Comments */
#.* ;							{
									ZVAL_STRINGL(token_value, yytext + 1, yyleng - 1);
									DBG_INF("QC_TOKEN_COMMENT");
									DBG_RETURN(QC_TOKEN_COMMENT);
								}

"--"[ \t].*						{
									ZVAL_STRINGL(token_value, yytext + 2, yyleng - 2);
									DBG_INF("QC_TOKEN_COMMENT");
									DBG_RETURN(QC_TOKEN_COMMENT);
								}
...
#endif
									*comment = mnd_ecalloc(1, sizeof(smart_string));
...
<COMMENT_MODE>"*/" 				{
									...
									smart_string_appendc(*comment, '\0');
									/*
									  we need to copy the smart_string by value before we set token_value
									...
									*/
									{
										smart_string * ss_copy = *comment;
										ZVAL_STRINGL(token_value, (*comment)->c, (*comment)->len);

										smart_string_free(ss_copy);
										mnd_efree(ss_copy);
									}
...
<COMMENT_MODE>.|\n 				{
									smart_string_appendc(*comment, yytext[0]);
								}
...
mysqlnd_qp_get_token(struct st_mysqlnd_query_scanner * scanner TSRMLS_DC)
{
	...
	if ((ret.token = yylex(&lex_val, *(yyscan_t *)scanner->scanner TSRMLS_CC))) {
		...
		switch (Z_TYPE(lex_val.zv)) {
			...
			case IS_NULL:
				if (lex_val.kn) {
					ZVAL_STRING(&ret.value, lex_val.kn);
				}
				break;
		}
	}
